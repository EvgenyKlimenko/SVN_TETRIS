...
public class WebSocketTetrisClient {
    ...
    private static final String SERVER = "ws://ecsc001014b4:8080/tetris-contest/ws";

    private static String userName = "EvgenyKlimenko";

    private static Pattern urlPattern = Pattern
	    .compile("^figure=(\\w+)&x=(\\d+)&y=(\\d+)&glass=(.*)&next=(\\w*)$");
	...
	connection = client.open(new URI(SERVER + "?user=" + userName),
		new WebSocket.OnTextMessage() {
			...
			String answer = solver.answer(matcher.group(1),
